...
export class MessageFormatter {
  public static async matrixEventToBody(
    event: MatrixMessageEvent,
    config: IConfigBridge,
    mediaProxy: MediaProxy,
  ): Promise<IBasicProtocolMessage> {
  ...
  public static async messageToMatrixEvent(
      ...
      if (matrixMsg.body.startsWith("<")) {
        ...
        try {
          ...
          if (md.markdown.length === 0 || md.html.length === 0) {
            throw new Error(
              "Markdown/HTML was zero length, which probably means it didn't parse well",
            );
          }

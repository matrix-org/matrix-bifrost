...
type XmppChatJoinComponents = { handle: string } & (
  | { room: string; server: string }
  | { fullRoomName: string }
);
...
export class XmppJsAccount implements IBifrostAccount {
  ...
  public createNew(_password?: string) {
    throw Error("Xmpp.js doesn't support registering accounts");
  }

  public setEnabled(_enable: boolean) {
  ...
  public sendIM(recipient: string, msg: IBasicProtocolMessage) {
    ...
    const res = this.xmpp.gateway
      ? this.xmpp.gateway.maskPMSenderRecipient(this.mxId, recipient)
      : null;
    ...
    const message = new StzaMessage(sender, recipient, msg, "chat");
  ...
  public sendChat(chatName: string, msg: IBasicProtocolMessage) {
    ...
    if (msg.formatted && msg.formatted.length) {
      msg.formatted.forEach((f) => {
        if (f.type === "html") {
          f.body = XHTMLIM.HTMLToXHTML(f.body);
        }
      });
    }
  ...
  public async selfPing(to: string): Promise<boolean> {
    ...
    const pingStanza = new StzaIqPing(`${this.remoteId}/${this.resource}`, to, id, "get");
    Metrics.remoteCall("xmpp.iq.ping");
    try {
      await this.xmpp.sendIq(pingStanza);
      return true;
    } catch (ex) {
      log.debug(`Self-ping to ${to} failed`, ex);
      return false;
    }
  }
  ...
  public async joinChat(
    ...
    const message = new StzaPresenceJoin(from, to);
    this.roomHandles.set(roomName, components.handle);
    if (setWaiting) {
  ...
  public async xmppRetryJoin(from: JID) {
    ...
    if (from.resource.endsWith(CONFLICT_SUFFIX)) {
      // Kick from the room.
      throw new Error(
        `A user with the prefix '${CONFLICT_SUFFIX}' already exists, cannot join to room.`,
      );
    }
  ...
  public async getAvatarBuffer(
    ...
    return {
      data: Buffer.from(photo.getChildText("BINVAL")!, "base64"),
  ...
  public setPresence(
    ...
    log.debug(
      `Broadcasting presence update ${this.remoteId} ${content.presence} ${recipients.join(", ")}`,
    );
    // Only allow bare JIDs
    const xmppRecipients = new Set(
      recipients.map((r) => jid(r)).filter((r) => r.bare().toString() === r.toString()),
    );
    this.xmpp.xmppSend(
      [...xmppRecipients]
        .map(
          (r) =>
            new StzaPresenceAvailable(
              this.remoteId,
              r.toString(),
              content.presence !== "online" ? "unavailable" : undefined,

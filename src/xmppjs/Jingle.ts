...
import { IStza, StzaIqResponse, StzaJingleAcceptIBB, StzaJingleTerminate } from "./Stanzas";
...
export abstract class JingleSession {
  constructor(
    ...
    public readonly sessionDescription: JingleSessionInitiateOpts,
  ) {
    /* */
  }
...
export class JingleFileTransferSession extends JingleSession {
  ...
  constructor(
    ...
    private readonly onUploadResult: (
      err: Error | null,
      mxcUrl?: string,
      file?: JingleFile,
    ) => void,
  ...
  public hashDigest() {
    this.assertState(
      JingleFileTransferSessionState.Writable,
      JingleFileTransferSessionState.Closed,
    );
...
export class JingleHandler extends EventEmitter {
  ...
  private async onSessionInitiate(
    jingleElement: Element,
    sid: string,
    from: string,
    to: string,
    iqId: string,
      ...
      case XMPPFeatures.JingleFileTransferV5:
        session = new JingleFileTransferSession(sid, info, this.uploadOpts, (err, mxcUrl, file) => {
          if (err) {
            ...
            // Tell the user.
            this.xmpp.sendIq(
              new StzaJingleTerminate(to, from, sid, undefined, undefined, "failed-application"),
            );
            return;
          }
    ...
    // Send an ACK
    await this.xmpp.xmppSend(new StzaIqResponse(to, from, iqId, "result"));
  ...
  public async onIBBStanza(stanza: Element) {
    ...
    const session = [...this.sessions.values()].find(
      (s) => s instanceof JingleFileTransferSession && s.transportSid === sid,
    ) as JingleFileTransferSession;
    if (!session) {
      // Not found
      // TODO: Improve error handling
      return this.xmpp.xmppSend(new StzaIqResponse(to, from, id, "error"));
    }
    if (openElement) {
    ...
    // ACK
    await this.xmpp.xmppSend(new StzaIqResponse(to, from, id, "result"));
  }
}

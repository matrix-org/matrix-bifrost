...
export class StzaPresence extends StzaBase {
  ...
  get xProtocol(): string {
    return "muc";
  }

  get presenceContent(): string {
    return "";
  }
  ...
  get xml(): string {
    ...
    if (this.includeXContent) {
      content = this.xContent
        ? `<x xmlns='http://jabber.org/protocol/${this.xProtocol}'>${this.xContent}</x>`
...
export class StzaPresenceAvailable extends StzaPresence {
  ...
  get presenceContent() {
    const hash = this.capsHash
      ? `<c xmlns='http://jabber.org/protocol/caps' hash='sha-1' node='${NODE_NAME}' ver='${encode(this.capsHash)}'/>`
      : "";
...
export class StzaPresenceError extends StzaPresence {
  ...
  public get presenceContent() {
    return (
      `<error type='${this.errType}' by='${this.by}'><${this.innerError}` +
      " xmlns='urn:ietf:params:xml:ns:xmpp-stanzas'/>" +
      (this.text
        ? `<text xmlns="urn:ietf:params:xml:ns:xmpp-stanzas">${encode(this.text)}</text>`
        : "") +
      "</error>"
    );
  }
...
export class StzaMessage extends StzaBase {
  ...
  constructor(
    ...
    if (idOrMsg && idOrMsg.hasOwnProperty("body")) {
      ...
      if (idOrMsg.opts) {
        ...
        this.attachments = (idOrMsg.opts.attachments || [])
          .filter((a) => "uri" in a)
          .map((a) => "uri" in a && a.uri);
      }
      this.id = idOrMsg.id;
  ...
  get xml(): string {
    ...
    const attachments = this.attachments.map(
      (a) => `<x xmlns='jabber:x:oob'><url>${encode(a)}</url></x>`,
    );
    ...
    // XEP-0308
    const replaces = this.replacesId
      ? `<replace id='${this.replacesId}' xmlns='urn:xmpp:message-correct:0'/>`
      : "";
    // XEP-0085
    const chatstate = this.chatstate
      ? `<${this.chatstate} xmlns='http://jabber.org/protocol/chatstates'/>`
      : "";
    const body = this.body ? `<body>${encode(this.body)}</body>` : "";
    return (
      `<message from="${this.from}" to="${this.to}" id="${this.id}" ${type}>` +
...
export class StzaMessageSubject extends StzaBase {
  ...
  get xml(): string {
    return (
      `<message from="${this.from}" to="${this.to}" id="${this.id}" type='groupchat'>` +
...
export class StzaJingleAcceptIBB extends StzaBase {
  ...
  get xml(): string {
    return (
      `<iq from='${this.from}' to='${this.to}' id='${this.id}' type='set'>` +
...
export class StzaIqPing extends StzaBase {
  ...
  get xml(): string {
    return (
      `<iq from='${this.from}' to='${this.to}' id='${this.id}' type='${this.responseType}'>` +
...
export class StzaIqPingError extends StzaIqPing {
  constructor(
    ...
    super(from, to, id, "error");
    this.extraContent =
      `<error type='cancel'${this.by ? ` by='${this.by}'` : ""}>` +
...
export abstract class StzaIqDisco extends StzaBase {
  ...
  get xml(): string {
    ...
    return (
      `<iq${node} from='${this.from}' to='${this.to}' id='${this.id}' type='${this.iqType}'>` +
...
export class StzaIqDiscoItems extends StzaIqDisco {
  ...
  constructor(from: string, to: string, id: string, queryType: string) {
  ...
  get queryContent(): string {
    const items = this.items
      .map((item) => `<item jid='${encode(item.jid)}' name='${encode(item.name)}'/>`)
      .join("");
    return items;
  }
...
export class StzaIqDiscoInfo extends StzaIqDisco {
  ...
  constructor(from: string, to: string, id: string, iqType = "result") {
  ...
  get hash() {
    ...
    s +=
      [...this.identity]
        ...
        .map((ident) => `${ident.category}/${ident.type}//${ident.name}`)
        .join("<") + "<";
...
export class StzaIqSearchFields extends StzaBase {
  ...
  get xml(): string {
    const fields = Object.keys(this.fields)
      .map((field) => `<${field}>${this.fields[field]}</${field}>`)
      .join("");
    return (
      `<iq from='${this.from}' to='${this.to}' id='${this.id}' type='result' xml:lang='en'>` +
...
export class SztaIqError extends StzaBase {
  ...
  get xml(): string {
    ...
    return (
      `<iq from='${this.from}' to='${this.to}' id='${this.id}' type='error' xml:lang='en'>` +
...
export class StzaIqVcardRequest extends StzaBase {
  ...
  get xml(): string {
    return (
      `<iq from='${this.from}' to='${this.to}' id='${this.id}' type='get'>` +

...
import {
  GatewayMUCMembership,
  IGatewayMemberXmpp,
  IGatewayMemberMatrix,
} from "./GatewayMUCMembership";
...
export class XmppJsGateway implements IGateway {
  ...
  constructor(
    ...
    private registration: AutoRegistration,
  ...
  public handleStanza(stanza: Element, gatewayAlias: string) {
    ...
    log.info(
      `Handling ${stanza.name} from=${stanza.attrs.from} to=${stanza.attrs.to} for ${gatewayAlias}`,
    );
  ...
  public sendMatrixMessage(
    ...
    const msgs = [...this.members.getXmppMembersDevices(chatName)].map(
      (device) => new StzaMessage(from.anonymousJid.toString(), device, msg, "groupchat"),
    );

    // add the message to the room history
    if (room.allowHistory) {
      const historyStanza = new StzaMessage(from.anonymousJid.toString(), "", msg, "groupchat");
   ...
   */
  public async reflectXMPPMessage(
    chatName: string,
    stanza: Element,
    kickNonMember = true,
  ): Promise<boolean> {
    ...
    if (!member && kickNonMember) {
      ...
      const kick = new StzaPresenceKick(stanza.attrs.to, stanza.attrs.from);
    ...
    try {
      ...
      this.roomHistory.addMessage(chatName, stanza, member.anonymousJid);
    } catch (ex) {
      log.warn(`Failed to add message for ${chatName} to history cache`, ex);
    }
    return true;
  }
  ...
  public reflectPM(stanza: Element) {
    ...
    const recipient = this.members.getMemberByAnonJid<IGatewayMemberXmpp>(
      convName,
      stanza.attrs.to,
    );
    if (!recipient) {
  ...
  public async sendMatrixMembership(chatName: string, event: MatrixMembershipEvent) {
    log.info(
      `Got new ${event.content.membership} for ${event.state_key} (from: ${event.sender}) in ${chatName}`,
    );
    // Iterate around each joined member and add the new presence step.
    const presenceEvents = GatewayStateResolve.resolveMatrixStateToXMPP(
      chatName,
      this.members,
      event,
    );
  ...
  public sendStateChange(
    ...
    this.reflectXMPPStanza(
      chatName,
      new StzaMessageSubject(
        chatName,
        "",
        undefined,
  ...
  public getMxidForRemote(sender: string) {
    ...
    return XMPP_PROTOCOL.getMxIdForProtocol(
      username,
      this.config.domain,
  ...
  public async onRemoteJoin(
      ...
      allMembershipPromises.push(
        (async () => {
          ...
          if (xmppMember.realJid) {
            ...
            realJid =
              this.registration.generateParametersFor(
                XMPP_PROTOCOL.id,
                (member as IGatewayMemberMatrix).matrixId,
              ).username +
              "/" +
              this.xmpp.defaultResource;
          }
    ...
    this.members.addXmppMember(`${to.local}@${to.domain}`, from, to, ownMxid);

    // 4. Room history
    if (room.allowHistory) {
      log.debug("Emitting history");
      const historyLimits: IHistoryLimits = {};
      const historyRequest = stanza
        .getChild("x", "http://jabber.org/protocol/muc")
        ?.getChild("history");
  ...
  public initialMembershipSync(chatName: string, room: IGatewayRoom, ghosts: BifrostRemoteUser[]) {
    ...
    for (const xmppUser of ghosts) {
      ...
      roomData.devices.forEach((device: string) =>
        this.members.addXmppMember(chatName, jid(device), jid(roomData.jid), xmppUser.id),
      );
    }
  }
  ...
  private updateMatrixMemberListForRoom(
    chatName: string,
    room: IGatewayRoom,
    allowForJoin = false,
  ) {
    ...
    for (const member of room.membership) {
      ...
      if (member.membership === "join") {
        ...
        left++;
        this.members.removeMatrixMember(chatName, member.stateKey);
      }
    }

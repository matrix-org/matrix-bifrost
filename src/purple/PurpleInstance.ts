...
export class PurpleInstance extends EventEmitter implements IBifrostInstance {
  ...
  public async start() {
    ...
    this.protocols = plugins
      .get_protocols()
      .map((data) => new PurpleProtocol(data, !!this.backendOpts.soloProtocol));
  ...
  public findProtocol(nameOrId: string): BifrostProtocol | undefined {
    return this.getProtocols().find(
      (protocol) =>
        protocol.name.toLowerCase() === nameOrId || protocol.id.toLowerCase() === nameOrId,
    );
  }
  ...
  private eventHandler() {
    helper.pollEvents().forEach((evt) => {
      ...
      if (evt.eventName === "chat-joined") {
        ...
        const purpleAccount = this.getAccount(
          chatJoined.account.username,
          chatJoined.account.protocol_id,
          "",
        );
        if (purpleAccount) {
      ...
      if (evt.eventName === "user-info-response") {
        ...
        const pAccount = this.accounts.get(
          `${uinfo.account.protocol_id}://${uinfo.account.username}`,
        );
        if (pAccount) {

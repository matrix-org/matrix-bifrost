...
export class PurpleAccount implements IBifrostAccount {
  ...
  constructor(
    ...
    public readonly protocol: BifrostProtocol,
  ) {
    this.enabled = false;
  ...
  get waitingJoinRoomProps(): IChatJoinProperties | undefined {
    return this.waitingJoinRoomProperties;
  }

  get remoteId(): string {
    return Util.createRemoteId(this.protocol.id, this.username);
  }

  get name(): string {
    return this.acctData!.username;
  }

  get handle(): PurpleAccountHandle {
    return this.acctData!.handle;
  }
    ...
    if (purple) {
      return new Promise((resolve, reject) => {
        ...
        cb = (ev: IConversationEvent) => {
          if (
            ev.account.username === this.username &&
            ev.account.protocol_id === this.protocol.id
          ) {
            resolve(ev);

...
export class AutoRegistration {
  static async create(
    autoRegConfig: IConfigAutoReg,
    accessConfig: IConfigAccessControl,
    bridge: Bridge,
    store: IStore,
    protoInstance: IBifrostInstance,
  ) {
    const cache: QuickLRU<string, Record<string, string>> = new QuickLRU({
      maxSize: autoRegConfig.registrationNameCacheSize,
    });
  ...
  public async registerUser(protocol: string, mxId: string): Promise<IBifrostAccount> {
    ...
    this.protoInstance
      .createBifrostAccount(res.username, proto)
      .createNew(res.extraParams.password);
  ...
  public async reverseRegisterUser(
    username: string,
    protocol: BifrostProtocol,
  ): Promise<IBifrostAccount> {
    ...
    if (usernameFormat.includes("<T_MXID>")) {
      ...
      if (!domainParameter) {
        throw Error(
          "`domain` must be specified in autoregistration parameters when only using a localpart",
        );
      }
      let regexStr = usernameFormat.replace(ESCAPE_TEMPLATE_REGEX, "\\$&");
    ...
    if (mxid) {
      ...
      try {
        await this.bridge.getIntent().getProfileInfo(mxid);
      } catch (ex) {
        throw Error("User doesn't exist", { cause: ex });
      }
    }
  ...
  private async handleHttpRegistration(mxId: string, step: IAutoRegStep) {
    ...
    try {
      ...
      if (profile.avatar_url) {
        profile.avatar_url = await intent.matrixClient.mxcToHttpThumbnail(
          profile.avatar_url,
          128,
          128,
          "crop",
    ...
    try {
      ...
      if (!opts.usernameResult) {
        // fetch it from the body.

...
export class RoomSync {
    ...
    await Promise.all(
      rooms.map(async (room) => {
        ...
        if (!this.bifrost.getProtocol(protocolId)) {
          log.debug(
            `Not syncing ${roomId} because the purple backend doesn't support this protocol`,
          );
          return;
        }
        ...
        if (isGateway) {
          ...
          try {
            ...
            log.warn(`Failed to sync gateway membership for room ${roomId}`, ex);
          }
          return;
        }
  ...
  private async syncMatrixUser(
    ...
    await ProtoHacks.addJoinProps(protocolId, props, userId, displayName || this.intent);
  ...
  private async onAccountSignedin(ev: IAccountEvent) {
    log.info(
      `${ev.account.username} signed in, checking if we need to reconnect them to some rooms`,
    );
    ...
    if (matrixUser === null) {
      ...
      try {
        ...
        log.warn(`Failed to disable ${ev.account.username}`, ex);
      }

      log.error(
        `Tried to get ${ev.account.username} but the backend found nothing. This shouldn't happen!`,
      );
      return;
    }
    ...
    log.debug(
      `Found ${reconsToMake} reconnections to be made for ${remoteId} (${matrixUser.getId()})`,
    );

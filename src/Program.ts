...
export class Program {
  ...
  constructor() {
    ...
    process.on("SIGTERM", () => this.killBridge());
  }
  ...
  private async initialiseMediaProxy(): Promise<MediaProxy> {
    ...
    const mediaProxy = new MediaProxy(
      { publicUrl, signingKey, ttl: config.ttlSeconds * 1000 },
  ...
  private async registerBot() {
    ...
    try {
      ...
      if (ex.errcode === "M_NOT_FOUND" || ex.errcode === "M_UNKNOWN") {
        return intent
          .setDisplayName(this.config.bridgeBot.displayname)
          .catch((err) => {
  ...
  private async pingBridge() {
    ...
    try {
      ...
      if (!internalRoom) {
        ...
        internalRoom = (
          await this.store.storeRoom(result.room_id, MROOM_TYPE_UADMIN, "-internal-", {
  ...
  public async runBridge(port: number, config: ConfigValue, registration?: AppServiceRegistration) {
    ...
    this.bridge = new Bridge({
      controller: {
        ...
        onAliasQuery: (alias, aliasLocalpart) =>
          this.eventHandler!.onAliasQuery(alias, aliasLocalpart),
        onEvent: (r) => {
        ...
        onEphemeralEvent: (r) => {
          ...
          if (data.type === "m.typing") {
            this.eventHandler
              .onTyping(r as Request<TypingEvent>)
              .catch((err) => {
            ...
            this.eventHandler
              .onPresence(r as Request<PresenceEvent>)
              .catch((err) => {
    ...
    if (
      this.config.autoRegistration.enabled &&
      this.config.autoRegistration.protocolSteps !== undefined
    ) {

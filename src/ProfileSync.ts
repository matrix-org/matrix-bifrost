...
export class ProfileSync {
  ...
  public async updateProfile(
    ...
    if (
      !force &&
      lastCheck != null &&
      Date.now() - lastCheck < this.config.profile.updateInterval
    ) {
    ...
    if (buddy === undefined) {
      try {
        ...
        log.info("Couldn't fetch user info for ", remoteUser.username, ex);
      }
    } else {
    ...
    if (
      remoteProfileSet.avatar_uri &&
      matrixUser.get("avatar_url") !== remoteProfileSet.avatar_uri
    ) {
  ...
  private async getOrCreateStoreUsers(
    protocol: BifrostProtocol,
    senderId: string,
  ): Promise<{ matrixUser: MatrixUser; remoteUser: BifrostRemoteUser }> {
    const userId: string = protocol
      .getMxIdForProtocol(senderId, this.config.bridge.domain, this.config.bridge.userPrefix)
      .getId();

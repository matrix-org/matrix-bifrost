...
export class MatrixRoomHandler {
  ...
  constructor(
    ...
    const handleAsyncEvent = function <Args = unknown, T = unknown>(
    ...
    purple.on(
      ...
      handleAsyncEvent(this.handleContactListSubscribeRequest.bind(this)),
    );
  ...
  private async isIMRoomStale(room: RoomBridgeStoreEntry): Promise<[string, Intent | null]> {
    ...
    const remoteIntent = this.bridge.getIntent(
      protocol
        .getMxIdForProtocol(recipient, this.config.bridge.domain, this.config.bridge.userPrefix)
        .getId(),
    );
    ...
    try {
      content = await remoteIntent.matrixClient.getRoomStateEventContent(
        roomId,
        "m.room.member",
        matrixUser,
      );
      ...
      switch (ex.statusCode) {
        case 403:
          log.warn(
            `IM room ${roomId} is stale because remote intent isn't a room member and never was`,
          );
          return [roomId, remoteIntent];
        case 404:
  ...
  private async getIMRoomId(
    data: { account: { protocol_id: string }; sender: string },
    ...
    const remoteEntry = await this.store.getIMRoom(
      matrixUser.getId(),
      data.account.protocol_id,
      data.sender,
    );
  ...
  private async createOrGetIMRoom(
    data: IReceivedImMsg,
    matrixUser: MatrixUser,
    intent: Intent,
  ): Promise<string> {
    ...
    log.info(
      `Couldn't find room for IM ${matrixUser.getId()} <-> ${data.sender}. Creating a new one`,
    );
    const remoteData = {
    ...
    try {
      ...
      if (
        this.config.tuning.waitOnJoinBeforePM.find((prefix) =>
          matrixUser.localpart.startsWith(prefix),
        )
      ) {
        log.info(
  ...
  private async createOrGetGroupChatRoom(
    ...
    const remoteId = Buffer.from(`${data.account.protocol_id}:${roomName}`).toString("base64");
    if (this.roomCreationLock.has(remoteId)) {
      log.info(remoteId, "is already being created, waiting...");
      await (this.roomCreationLock.get(remoteId) || Promise.resolve());
      log.info("room was created, no longer waiting");
  ...
  private async handleIncomingIM(data: IReceivedImMsg) {
    ...
    const account = this.purple.getAccount(
    ...
    try {
      ...
      log.warn("Not joined to room, discarding " + roomId, ex);
      await this.store.removeRoomByRoomId(roomId);
      roomId = await this.createOrGetIMRoom(data, matrixUser, intent);
    }
    ...
    if (data.message.original_message) {
      data.message.original_message =
        (await this.store.getMatrixEventId(roomId, data.message.original_message)) || undefined;
    }
  ...
  private async handleIncomingChatMsg(data: IReceivedImMsg) {
    ...
    if (this.accountRoomLock.has(acctId + "/" + data.conv.name)) {
    ...
    if (
      this.purple.needsDedupe() &&
      (await this.deduplicator.checkAndRemove(data.conv.name, remoteId, data.message.body))
    ) {
      return;
    }
    ...
    if (
      this.purple.needsDedupe() &&
      !this.deduplicator.isTheChosenOneForRoom(data.conv.name, acctId)
    ) {
      return;
    }
  ...
  private async handleRemoteUserState(data: IUserStateChanged) {
    ...
    const profileNeeded =
      this.config.tuning.waitOnProfileBeforeSend && (!remoteUser || remoteUser!.displayname);
    try {
      if (data.state === "joined") {
        if (!profileNeeded) {
          await intent.join(roomId);
  ...
  private async handleIMTyping(data: IChatTyping) {
    ...
    const intent = this.bridge.getIntent(
      this.purple
        .getProtocol(data.account.protocol_id)!
        .getMxIdForProtocol(data.sender, this.config.bridge.domain, this.config.bridge.userPrefix)
        .userId,
    );
  ...
  private async handleReadReceipt(data: IChatReadReceipt) {
    ...
    const roomId = await this.createOrGetGroupChatRoom(data, intent, true);
    const eventId = data.originIsMatrix
      ? data.messageId
      : this.remoteEventIdMapping.get(data.messageId);
    if (!eventId) {

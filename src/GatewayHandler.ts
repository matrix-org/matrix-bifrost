...
function isHistoryVisibilitySafe(historyVisibility: unknown): boolean {
  return HISTORY_SAFE_ENUMS.includes(
    typeof historyVisibility === "string" ? historyVisibility : "joined",
  );
}
...
export class GatewayHandler {
  ...
  public async getVirtualRoom(roomId: string, intent: Intent): Promise<IGatewayRoom> {
    ...
    const promise = (async () => {
      ...
      log.debug(
        `Hydrated room ${roomId} '${room.name}' '${room.topic}' ${room.membership.length} `,
      );
      return room;
    })();
  ...
  private async handleRoomJoin(data: IGatewayJoin) {
    ...
    try {
      ...
      if (canonAlias !== data.roomAlias) {
        throw Error("We do not support multiple room aliases, try " + canonAlias);
      }
  ...
  private async handlePublicRooms(ev: IGatewayPublicRoomsQuery) {
    log.info(
      `Trying to discover public rooms search=${ev.searchString} homeserver=${ev.homeserver}`,
    );
    try {
  ...
  private async getOrCreateGatewayRoom(
    data: IGatewayJoin,
    roomId: string,
  ): Promise<RoomBridgeStoreEntry> {
    const remoteId = Buffer.from(`${data.protocol_id}:${data.room_name}`).toString("base64");

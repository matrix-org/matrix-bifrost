...
export class MatrixEventHandler {
  ...
  private readonly pendingRoomAliases: Map<
    string,
    { protocol: BifrostProtocol; props: IChatJoinProperties }
  >;
  constructor(
  ...
  public async onAliasQueried(alias: string, roomId: string) {
    ...
    const remoteId = Buffer.from(`${protocol.id}:${remoteData.room_name}`).toString("base64");
    try {
  ...
  public async onEvent(request: Request<WeakEvent>) {
    ...
    if (
      ...
      event.state_key?.length &&
      typeof event.content.membership === "string"
    ) {
    ...
    log.debug(
      "Got event (id, type, sender, state_key, roomtype):",
      event.event_id,
      event.type,
      event.sender,
      event.state_key,
    ...
    if (newInvite && event.state_key) {
      ...
      if (event.state_key === botUserId) {
        ...
        const { username, protocol } = this.purple.getUsernameFromMxid(
          event.state_key!,
        ...
        const remoteId = Buffer.from(`${event.sender}:${protocol.id}:${username}`).toString(
          "base64",
        );
        const { remote } = await this.store.storeRoom(
          event.room_id,
          MROOM_TYPE_IM,
          remoteId,
          remoteData,
    ...
    if (membershipEvent) {
      switch (roomType) {
        ...
        case MROOM_TYPE_IM:
          if (
            membershipEvent.content.membership === "leave" &&
            membershipEvent.sender === ctx.remote.get<string>("matrixUser")
          ) {
            ...
            log.info(
              `Left and removed entry for IM room ${membershipEvent.room_id} because the user left`,
            );
            return;
          }
      }
  ...
  public async onTyping(r: Request<TypingEvent>) {
    ...
    const isUserTyping = !!typing.content.user_ids.filter(
      (u) => !this.bridge.getBot().isRemoteUser(u),
    )[0];
  ...
  async onPresence(req: Request<PresenceEvent>) {
    ...
    for (const remoteUser of remoteUsers) {
      try {
        log.debug(
          `Sending presence on behalf of Matrix user via ${remoteUser.protocolId}:${remoteUser.username}`,
        );
        ...
        log.warn(
          `Failed to handle presence update for ${data.sender} via account ${remoteUser.protocolId}:${remoteUser.username}`,
          ex,
        );
      }
    }
  ...
  private async handleCommand(args: string[], event: WeakEvent) {
    ...
    if (args[0] === "protocols" && args.length === 1 && !soloProtocol) {
      ...
      body += protocols
        .map((plugin: BifrostProtocol) => ` \`${plugin.name}\` - ${plugin.summary}`)
        .join("\n\n");
      ...
      body += users
        .map((remoteUser: BifrostRemoteUser) => {
          let account: IBifrostAccount | null = null;
          try {
            account = this.purple.getAccount(
              remoteUser.username,
              remoteUser.protocolId,
              event.sender,
            );
          } catch (ex) {
      ...
      if (soloProtocol) {
        ...
        body +=
          "- `accounts add $USERNAME $PASSWORD` Add a new account, this will take some options given.\n";
  ...
  private async handlePlumbingCommand(args: string[], event: WeakEvent) {
    ...
    const userPl =
      powerLevels.users[event.sender] === undefined
        ? powerLevels.users_default
        : powerLevels.users[event.sender];
    if (userPl < requiredPl) {
      log.warn(
        `${event.sender}'s PL is too low to run a plumbing command ${userPl} < ${requiredPl}`,
      );
      return;
    }
    try {
      if (args[1] === "bridge") {
        ...
        const paramSet = await this.getJoinParametersForCommand(
          acct,
          cmdArgs,
          event.room_id,
          "!purple bridge",
        ...
        if (paramSet != null) {
          ...
          const remoteId = Buffer.from(`${acct.protocol.id}:${res.conv.name}`).toString("base64");
  ...
  private async handleInviteForBot(event: WeakEvent) {
    ...
    log.info(`Created new 1 to 1 admin room for ${event.sender} ${event.room_id}`);
  }
  ...
  private async handleEnableAccount(
    protocolId: string,
    username: string,
    mxid: string,
    enable: boolean,
  ...
  private async handleGroupMessage(context: RoomBridgeStoreEntry, event: WeakEvent) {
    ...
    try {
      ...
      try {
        ...
        nick =
          conv && this.purple.getNickForChat
            ? this.purple.getNickForChat(conv) || acct.name
            : acct.name;
      } catch (ex) {
        log.debug(`Failed to determine nick for ${roomName}, falling back to account name`, ex);
        nick = acct.name;
      }
      if (this.purple.needsDedupe()) {
        this.deduplicator.insertMessage(
          roomName,
          Util.createRemoteId(roomProtocol, ProtoHacks.getSenderId(acct, nick, roomName)),
          msg.body,
        );
      }
  ...
  private async handleJoinLeaveGroup(context: RoomBridgeStoreEntry, event: MatrixMembershipEvent) {
    ...
    if (isGateway) {
      try {
        await this.gatewayHandler.sendMatrixMembership(name, context, event);
      } catch (ex) {
    ...
    try {
      ...
      if (membership === "join") {
        await this.bridge
          .getIntent()
          .kick(event.room_id, event.sender, "Could not find a compatible purple account.");
      }
      return;
    }
  ...
  private async handleStateEv(context: RoomBridgeStoreEntry, event: WeakEvent) {
    ...
    if (isGateway) {
      try {
        await this.gatewayHandler.sendStateEvent(name, event.sender, event, context);
      } catch (ex) {
  ...
  private async getJoinParametersForCommand(
    acct: IBifrostAccount,
    args: string[],
    roomId: string,
    command: string,
    ...
    if (args.length === 1) {
      ...
      const body = `The following **required** parameters must be specified in order.
...
`;
  ...
  private async joinOrDefer(
    ...
    const joinEvent = await new Promise<IAccountEvent>((resolve, reject) => {
      let fn;
      fn = (ev: IAccountEvent) => {
        if (ev.account.username === acct.name && acct.protocol.id === ev.account.protocol_id) {
          resolve(joinEvent);
        }

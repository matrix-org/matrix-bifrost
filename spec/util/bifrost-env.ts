...
import {
    createContainers, createDatabase, dropDatabase, TestContainerNetwork,
} from "./containers";
import {
    XMPP_COMPONENT_DOMAIN, XMPP_COMPONENT_SECRET, XMPP_C2S_DOMAIN,
    XMPP_TEST_USER, XMPP_TEST_PASSWORD, XMPP_TEST_RESOURCE,
...
async function generateSigningKey(dir: string): Promise<string> {
    const key = await webcrypto.subtle.generateKey({ name: "HMAC", hash: "SHA-512" }, true, ["sign", "verify"]);
...
async function registerMatrixUser(
    homeserverUrl: string, sharedSecret: string, localpart: string,
): Promise<{mxid: string, accessToken: string}> {
    const registerUrl = `${homeserverUrl}/_synapse/admin/v1/register`;
    const nonce = await fetch(registerUrl).then(r => r.json()).then((j: {nonce: string}) => j.nonce);
...
export class BifrostTestEnv {
    ...
    public async setUp(opts: BifrostTestEnvOpts = {}): Promise<void> {
        ...
        for (const localpart of opts.matrixLocalparts ?? []) {
            const { accessToken } = await registerMatrixUser(synapse.baseUrl, synapse.registrationSecret, localpart);

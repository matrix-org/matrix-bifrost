...
export class E2ETestMatrixClient extends MatrixClient {
  public async waitForRoomEvent(opts: {
    eventType: string;
    sender: string;
    roomId?: string;
    stateKey?: string;
    ...
    return this.waitForEvent(
      "room.event",
      (eventRoomId: string, eventData: RoomEvent) => {
  ...
  public async waitForRoomInvite(opts: {
    ...
    return this.waitForEvent(
      "room.invite",
      (eventRoomId: string, eventData: { sender: string }) => {
  ...
  public async waitForRoomJoin(opts: { roomId?: string }): Promise<{ roomId: string }> {
    const { roomId } = opts;
    return this.waitForEvent(
      "room.join",
      (eventRoomId: string) => {
  ...
  public async waitForEvent<T>(
    emitterType: string,
    filterFn: (...args: never[]) => T | undefined,
    timeoutMsg: string,
  ): Promise<T> {

...
export async function createPublicRoom(
  testEnv: BifrostTestEnv,
  alice: E2ETestMatrixClient,
...
export function collectStanzas(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out waiting for ${count} stanzas (got ${collected.length}): ${description}`,
        ),
      );
    }, timeoutMs);
    xmpp.on("stanza", onStanza);
...
export async function joinGateway(testEnv: BifrostTestEnv, joinTo: string): Promise<void> {
  const selfPresence = waitForStanza(
    testEnv.xmpp,
    "self-presence confirming gateway join",
    (s) => isPresenceFrom(s, joinTo) && hasMucStatusCode(s, "110"),
  );
  await testEnv.xmpp.send(
    xml("presence", { to: joinTo }, xml("x", { xmlns: "http://jabber.org/protocol/muc" })),
  );
  await selfPresence;
}
...
export class OccupantTracker {
  ...
  constructor(
    private xmpp: ReturnType<typeof xmppClient>,

import {
  ProsodyContainer,
  XMPP_TEST_USER,
  type StartedProsodyContainer,
  XMPP_TEST_PASSWORD,
} from "./prosody";
import { SynapseContainer, type StartedSynapseContainer } from "./synapse";

import { TestContainers, Network, StartedNetwork } from "testcontainers";
...
export async function createContainers(
  ...
  const [synapse, prosody, postgres] = await Promise.all([
    new SynapseContainer(serverName)
      .withNetwork(network)
...
export async function createDatabase(
  postgres: StartedPostgreSqlContainer,
): Promise<{ name: string; connectionString: string }> {

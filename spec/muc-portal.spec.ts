 ...
 */
async function joinAsOwnerAndSetName(
...
describe("XMPP MUC portal rooms", () => {
  test("names the Matrix portal room after the MUC's disco#info identity name", async ({
    testEnv,
    alice,
  }) => {
  ...
  test("keeps a portal room's existing name when the MUC subject is delivered on join", async ({
    testEnv,
    alice,
  }) => {
    ...
    const nameContent = (await alice.getRoomStateEvent(roomId, "m.room.name", "")) as {
      name: string;
    };

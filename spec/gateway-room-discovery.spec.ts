...
describe("XMPP gateway", () => {
  test("lists public Matrix rooms via disco#items", async ({ testEnv, alice }) => {
    ...
    const response = await testEnv.xmpp.iqCaller.request(
      xml(
        "iq",
    ...
    const items =
      response.getChild("query", "http://jabber.org/protocol/disco#items")?.getChildren("item") ??
      [];
  ...
  test("returns disco#info for a gateway room JID, naming it after the Matrix room", async ({
    testEnv,
    alice,
  }) => {
  ...
  test("returns item-not-found for a gateway room JID with no matching Matrix alias", async ({
    testEnv,
  }) => {
    const to = roomJid(`#does-not-exist:${testEnv.serverName}`);

    await expect(
      testEnv.xmpp.iqCaller.request(
        xml(
          "iq",

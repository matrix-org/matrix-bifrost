...
describe("XMPP gateway history backfill", () => {
  test("replays cached messages, in order, to an XMPP user rejoining a history-visible room", async ({
    testEnv,
    alice,
  }) => {
    const bobNick = "bob";
    ...
    const firstToXmpp = waitForStanza(
      testEnv.xmpp,
      "first live message",
      (s) => s.is("message") && s.getChildText("body") === "first message",
    );
    await alice.sendMessage(roomId, { msgtype: "m.text", body: "first message" });
    await firstToXmpp;

    const secondToMatrix = alice.waitForRoomEvent({
    ...
    const thirdToXmpp = waitForStanza(
      testEnv.xmpp,
      "third live message",
      (s) => s.is("message") && s.getChildText("body") === "third message",
    );
  ...
  test("does not replay history into a room whose history visibility isn't shared", async ({
    testEnv,
    alice,
  }) => {
    const bobNick = "bob";
    ...
    await alice.sendStateEvent(roomId, "m.room.history_visibility", "", {
    ...
    const messageDelivered = waitForStanza(
      testEnv.xmpp,
      "live message",
      (s) => s.is("message") && s.getChildText("body") === "should stay private",
    );
    ...
    const history = collectStanzas(
      testEnv.xmpp,
      "replayed history on rejoin (should not happen)",
      isDelayedGroupchatMessage,
      1,
      3000,
    );
  ...
  test("picks up a live history_visibility change without a restart, only caching what followed it", async ({
    testEnv,
    alice,
  }) => {
    const bobNick = "bob";
    ...
    const alias = await createPublicRoom(
      testEnv,
      alice,
      "gateway-history-visibility-change-room",
      "Gateway History Visibility Change Room",
    );
    const roomId = await alice.resolveRoom(alias);
    ...
    const unexpectedSecondMessage = collectStanzas(
      testEnv.xmpp,
      "a second replayed message (should not happen)",
      isDelayedGroupchatMessage,
      1,
      3000,
    );

...
describe("Stanzas", () => {
  ...
  describe("StzaPresenceError", () => {
    it("should create a valid stanza", () => {
      const xml = new StzaPresenceError(
        "foo@bar",
        "baz@bar",
        "someid",
        "baz2@bar",
  ...
  describe("StzaPresencePart", () => {
    it("should create a valid stanza", () => {
      ...
      assertXML(xml);
      expect(xml).toBe('<presence from="foo@bar" to="baz@bar" type=\'unavailable\'></presence>');
    });
  });
  describe("StzaPresenceKick", () => {
    it("should create a valid stanza", () => {
      const xml = new StzaPresenceKick("foo@bar", "baz@bar", "reasonable reason", "Kicky", true)
        .xml;
      assertXML(xml);
      expect(xml).toBe(
  ...
  describe("StzaIqDiscoInfo", () => {
    ...
    it("should create a valid hash", () => {
      ...
      expect(userDiscoInfo.hash).toBe("YvWxpAh3qsnZdItZNak8ruVh+Gs=");
    });
  });
});

...
describe("XHTMLIM", () => {
  it("should not change compliant messages", () => {
    ...
    ).toBe("<html xmlns='http://jabber.org/protocol/xhtml-im'><p>Hello world</p></html>");
  });
  it("should transform a simple text message", () => {
    expect(XHTMLIM.HTMLToXHTML("o/")).toBe(
      "<html xmlns='http://jabber.org/protocol/xhtml-im'>o/</html>",
    );
  });
  it("should transform a message with a link", () => {
    expect(
      XHTMLIM.HTMLToXHTML('<a href="https://matrix.to/#/@bob:matrix.org">bob</a>: Huzzah!'),
    ).toBe(
  ...
  it("should transform an inline image", () => {
    const xhtmlValue = XHTMLIM.HTMLToXHTML(
      'Here is a pretty image<span class="d-emoji"><img alt="shadow" title="shadow" height="32" src="http://foobar.com" /></span>',
    );
    assertXML(xhtmlValue);
    expect(xhtmlValue).toBe(
      "<html xmlns='http://jabber.org/protocol/xhtml-im'>Here is a pretty image<span class='d-emoji'>" +

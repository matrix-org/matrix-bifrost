...
import {
  ...
  XMPP_MEMBER_JID,
  XMPP_MEMBER_JID_SECOND_DEVICE,
  XMPP_MEMBER_JID_STRIPPED,
  XMPP_MEMBER_MXID,
} from "./fixtures";
...
describe("GatewayMUCMembership", () => {
  ...
  describe("adding members", () => {
    it("can add a XMPP member", () => {
      const firstDevice = members.addXmppMember(
        XMPP_CHAT_NAME,
        XMPP_MEMBER_JID,
        XMPP_MEMBER_ANONYMOUS,
        XMPP_MEMBER_MXID,
    ...
    it("can add a Matrix member", () => {
      const firstDevice = members.addMatrixMember(
        XMPP_CHAT_NAME,
        MATRIX_MEMBER_MXID,
        XMPP_MEMBER_ANONYMOUS,
      );
  ...
  describe("removing members", () => {
    ...
    it("can add two devices, and remove one", () => {
      ...
      lastDevice = members.removeXmppMember(
        XMPP_CHAT_NAME,
        XMPP_MEMBER_JID_SECOND_DEVICE.toString(),
      );
      expect(lastDevice).toBe(true);
    });
  ...
  describe("finding members", () => {
    ...
    it("will not find an XMPP member by a second device JID", () => {
      ...
      expect(
        members.getXmppMemberByDevice(XMPP_CHAT_NAME, XMPP_MEMBER_JID_SECOND_DEVICE),
      ).toBeUndefined();
    });

...
describe("Util", () => {
  ...
  describe("getMxIdForProtocol", () => {
    ...
    it("should create a simple userId", () => {
      ...
      expect(mxUser.getId()).toBe("@_purple_protocol_simple:example.com");
    });
    it("should create a sensible userId from a sender containing url parts", () => {
      ...
      expect(mxUser.getId()).toBe("@_purple_protocol_fred=40banana.com:example.com");
    });
    it("should create a sensible userId from a sender containing a matrix userid", () => {
      const mxUser = protocol.getMxIdForProtocol("@fred:banana.com", "example.com", "_purple_");
      expect(mxUser.getId()).toBe("@_purple_protocol_=40fred=3abanana.com:example.com");
    });
    it("should create a sensible userId for an xmpp jid", () => {
      const mxUser = XMPP_PROTOCOL.getMxIdForProtocol(
        "frogman@frogplanet.com",
        "example.com",
        "_xmpp_",
      );
      expect(mxUser.getId()).toBe("@_xmpp_frogman=40frogplanet.com:example.com");
    });
    it("should create a sensible userId for an xmpp jid with a resource", () => {
      const mxUser = XMPP_PROTOCOL.getMxIdForProtocol(
        "frogman@frogplanet.com/frogdevice",
        "example.com",
        "_xmpp_",
      );
      expect(mxUser.getId()).toBe("@_xmpp_frogdevice=2ffrogman=40frogplanet.com:example.com");
    });
    it("should create a sensible userId for an xmpp jid with a resource with special chars", () => {
      const mxUser = XMPP_PROTOCOL.getMxIdForProtocol(
        "frogman@frogplanet.com/Frog!%$£ device",
        "example.com",
        "_xmpp_",
      );
      expect(mxUser.getId()).toBe(
        "@_xmpp_Frog=21=25=24=a3=20device=2ffrogman=40frogplanet.com:example.com",
      );
    });
  });
  ...
  describe("unescapeUserId", () => {
    it("should unescape QF encoding", () => {
      expect(Util.unescapeUserId("Hello=a3=21=25=26=20World")).toBe("Hello£!%& World");
    });
  });
});
